...
霰弹枪的 `13` 枚可见弹粒仅用于弹道表现；每个扇形内有效目标仍按距离结算一次。无模块时，所有枪械继续使用上述原始行为参数。生存模式下服务端从主背包或副手扣除对应弹药；创造模式不扣弹药。铁砧使用铁锭维修，Smithing Table 使用 `upgrade_template` 和固定模块进行最多三次不同模块安装。

### 弹道视觉不变量

- 命中、伤害、穿透、爆炸和终点由服务端计算；粒子只表现结果，不参与玩法判定。
- 狙击枪使用紧凑灰色实体弹粒，不保存或推进命中后的假弹丸；视觉速度固定为 `0.110` blocks/tick，是上一版的两倍。
- 霰弹枪使用中心弹粒加内外双环共 13 枚运动弹粒，每条最多 5 个灰色范围点；SMG 使用黑色实体弹粒且不生成枪口大闪光。
- 火焰喷射器每次只从枪口发射 6 个有速度和寿命的粒子，由连续射击叠加成稳定火流，不预铺整条静态锥体。
- 轨道炮生成带明亮核心和柔和外晕的巨大白色激光，客户端生命周期固定 `60` ticks 并逐 tick 淡出；枪口起点与视点保持安全偏移，命中点继续使用小型白色粒子，避免整屏闪光。
- 空气中使用 Guns 自定义粒子；采样点落入水体时改用气泡或弱化能量表现。
- 不新增 Guns Payload，复用 Minecraft 原生服务器粒子同步。
- 单次硬上限：Sniper `80` 个采样，Shotgun 最坏 `91` 次且绝不超过 `96`，SMG `12`，Flamethrower `6` 个移动粒子，Grenade trail 每 tick `3` 个调用，Railgun 在 4 个可见实体命中点下最坏 `204` 个调用且绝不超过 `208`。Railgun 冷却和光束寿命同为 60 ticks，不会无界叠加；SMG 在每秒 10 发时仍限制为最多 `120` 次粒子调用/秒。
