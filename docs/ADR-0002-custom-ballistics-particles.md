...
后续实机反馈进一步要求轨道炮从细束升级为大型激光并延长到 3 秒渐隐、狙击枪视觉速度提高 2 倍、霰弹枪明确展示弹丸向外散开。该修订仍属于 Option B 的参数和纹理演进，不需要新增 Particle ID、Payload 或第三方渲染依赖。
...
- 喷火器采用本地 Twilight Forest Hydra 源码所示的连续运动粒子原则；轨道炮使用大尺寸径向白色核心/外晕纹理并固定 60 ticks 淡出，音效复用 Guardian 与 Beacon 原版事件。
...
- `runGameTest` 验证弹药、升级、配方和服务端行为，并锁定狙击速度、霰弹与轨道炮预算以及 60-tick 光束寿命。
